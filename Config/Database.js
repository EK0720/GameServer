...
const DatabaseSettings = require('../Files/DatabaseSettings.json');
const DatabaseSettingsLocal = require('../Files/DatabaseSettingsLocal.json');
...
module.exports = class Database {
    constructor(isLocal = false) {
        ...
        this.pool = new Pool({
            ...
            // Add port for PostgreSQL, typically 5432
            port: 5432
        });
    }
              ...
              client.query(query, [userId], (error, results) => {
                  ...
                  if (error) {
                      ...
                      if (results.rowCount > 0) {
                          callback({ valid: true, reason: "User deleted successfully." });
                      } else {
                          callback({ valid: false, reason: "User not found." });
                      }
                  }
              });
          }
  ...
  UpdateById(id, email, username, password, displayName, callback) {
      this.Connect((error, client) => {
          if (error) {
              ...
              query = query.slice(0, -2) + " WHERE id = $5";
              values.push(id);
    ...
    CreateAccount(username, password, email, callback) {
      this.Connect((error, client) => {
          if (error) {
              ...
              let checkQuery = "SELECT COUNT(*) AS count FROM users WHERE username = $1 OR email = $2";
              client.query(checkQuery, [username, email], (error, results) => {
                  client.release();
                  if (error) {
                      callback(error);
                  } else {
                      ...
                      if (count > 0) {
                          ...
                          let insertQuery = "INSERT INTO users (id, username, password, email, displayName, createTime) VALUES ($1, $2, $3, $4, NULL, $5)";
                          client.query(insertQuery, [id, username, hashedPassword, email, createTime], (error, results) => {
                              if (error) {
                                  callback(error);
                              } else {
                                  if (results.rowCount > 0) {
              ...
              if (error) {
                  ...
                  if (results[0] != undefined) {
                      ...
                      if (PasswordHash.verify(currentPassword, hashedPassword)) {
                          let updateQuery = "UPDATE users SET password = $1 WHERE username = $2";
                          client.query(updateQuery, [PasswordHash.generate(newPassword), username], (error, results) => {
                              if (error) {
                                  callback(error);
                              } else {
                                  callback({
...
ForgotPassword(email, callback) {
  this.Connect((error, client) => {
      if (error) {
          ...
          let query = "SELECT username, password FROM users WHERE email = $1";
          client.query(query, [email], (error, results) => {
              client.release();
              if (error) {
                  callback(error);
              } else {
                  if (results.rows[0] != undefined) {
                      ...
                      client.query(updateQuery, [PasswordHash.generate(newpassword), email], (error, results) => {
                          if (error) {
                              callback(error);
                          } else {
...
GetUsername(username, callback) {
  this.Connect((error, client) => {
      if (error) {
          ...
          let query = "SELECT displayName FROM users WHERE username = $1";
          client.query(query, [username], (error, results) => {
              client.release();
              if (error) {
                  callback(error);
                  ...
                  if (results.rows[0] != undefined) {
                      callback(results.rows[0].displayName);
                  } else {
                      callback(null);
                  }
              }
    ...
    sendFriendRequest(senderUsername, friendUsername, callback) {
      ...
      this.Connect((error, client) => {
          if (error) {
              ...
              let query = "UPDATE users SET displayName = $1 WHERE username = $2";
              client.query(query, [displayName, username], (error, results) => {
                  client.release();
  ...
  SwitchUserOnlineStatus(username, onlineStatus, callback) {
      this.Connect((error, client) => {
          if (error) {
              ...
              let query = "UPDATE users SET online = $1 WHERE username = $2";
              client.query(query, [onlineStatus, username], (error, results) => {
                  client.release();
  ...
  sendFriendRequest(senderUsername, friendUsername, callback) {
      this.Connect((error, client) => {
          if (error) {
              ...
              const friendshipId = uuid.v4();
              let query = "INSERT INTO friendships (friendship_id, user_id, friend_id) SELECT $1, u1.id, u2.id FROM users u1, users u2 WHERE u1.username = $2 AND u2.username = $3";
  
              client.query(query, [friendshipId, senderUsername, friendUsername], (error, results) => {
                  client.release();
