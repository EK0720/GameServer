...
module.exports = class Database {
    ...
    checkFriendRequest(username, callback) {
      this.Connect((connection) => {
        let query = "SELECT f.*, u1.username AS user_username, u2.username AS friend_username FROM friendships f INNER JOIN users u1 ON f.user_id = u1.id INNER JOIN users u2 ON f.friend_id = u2.id WHERE u1.username = ? OR u2.username = ?";
          ...
          const friendNames = results.map((friendship) => {
            if (friendship.user_username === username) {
              if (friendship.status === 0) {
                return `${friendship.friend_username}: Friend request sent`;
              } else if (friendship.status === 1) {
                return `${friendship.friend_username}: Accepted`;
              } else {
                return `${friendship.friend_username}: Pending`;
              }
            } else {
              if (friendship.status === 0) {
                return `${friendship.user_username}: Friend request received`;
              } else if (friendship.status === 1) {
                return `${friendship.user_username}: Accepted`;
              } else {
                return `${friendship.user_username}: Pending`;
              }
            }
          });
  ...
  getMessagesByUsername(senderName, receiverName, callback) {
    this.Connect(connection => {
      let query = "SELECT m.* FROM messages m INNER JOIN users s ON m.sender_id = s.id INNER JOIN users r ON m.receiver_id = r.id WHERE (s.username = ? AND r.username = ?) OR (s.username = ? AND r.username = ?)";
